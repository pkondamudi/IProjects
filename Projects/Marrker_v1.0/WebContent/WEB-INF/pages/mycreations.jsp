...
<head>
<meta charset="utf-8">
<title>My Creations - {Marrker}</title>
			...
			<c:when test="${fn:length(userBoards) > 0}">
				<c:forEach var="userBoard" items="${userBoards}">
					<div class="row">
						<div class="card">
							...
							<div class="card-actions">
								<div class="btn-group">
									...
									<c:if test="${userBoard.isDefault eq 0}">
										<button type="button"
										...
										<ul class="dropdown-menu pull-right" role="menu">
											<%-- <li><a href="#editModal_${userBoard.boardId}"
											role="button" data-toggle="modal">Edit</a></li> --%>
											<li><a href="ctx/deleteContext/${userBoard.boardId}">Delete</a></li>
										</ul>
									</c:if>
								</div>
							</div>
						...
						<!-- Modal -->
						<div id="editModal_${userBoard.boardId}" class="modal hide fade"
							...
							<div class="modal-body">
								<form:form method="post" action="ctx/editContext"
									commandName="contextBoardObj" name="editContextBoard">
									<input type="hidden" name="boardId"
										value="${userBoard.boardId }">
									<input type="hidden" name="coverImageLocation"
										value="${userBoard.coverImageLocation }">
									<fieldset>
										<legend>Edit Context</legend>
										<label>Code:</label> <input type="text"
											placeholder="Enter your context code..." style="width: 100%"
											value="${userBoard.boardName }" name="BoardName"> <span
											class="help-block">Example: <b>Fun2016</b>.(do
											not put any spaces in between)
										</span> <label>Type</label> <select style="width: 100%"
											name="boardType">
											<option value="Protected" selected>Protected
												(Recommended) - Users who follow can also contribute</option>
											<option value="Private">Private - Only you can
												contribute the information.</option>
											<option value="Public">Public - Any Marrker
												user can contribute.</option>
										</select> <label>Description:</label>
										<textarea type="text"
											placeholder="Enter your context description..."
											style="width: 100%" rows="4" cols="25"
											name="BoardDescription">${userBoard.boardDescription }</textarea>
									</fieldset>
							</div>
				...
				<ul class="pager">
					<li><a href="userCreations?nav=pre">Previous</a></li>
					<li><a href="userCreations?nav=nxt">Next</a></li>
				</ul>
			</c:when>
			<c:otherwise>
				<div class="card">
					<div class="card-body">
						<p style="padding: 0px 0px 10px 0px">No contexts found. Go to
							Site Options and start posting.</p>
					</div>
				</div>
			</c:otherwise>
		</c:choose>
			...
			<form:form method="post" action="ctx/createContext"
				...
				<fieldset>
					...
					<label>Code:</label> <input type="text"
						...
						<option value="Public">Public - Any Marrker user
							can contribute.</option>
					</select> <label>Description:</label>
					<textarea type="text"
						placeholder="Enter your context description..."
