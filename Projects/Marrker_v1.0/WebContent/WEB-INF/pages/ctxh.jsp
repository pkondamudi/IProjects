...
<body>
				...
				<ul class="dropdown-menu">
					...
					<c:choose>
						<c:when test="${currentContext.isUserFollowing }">
							<li><a href="unFollow/${ctxid}">Unfollow</a></li>
						</c:when>
						<c:otherwise>
			...
			<div class="tabbable tabs-left">
				...
				<div class="tab-content">
					<div class="tab-pane active" id="tab1">
						<p>
							<form:form method="post" action="doCtxPost" name="ctxPost"
								commandName="ctxPost" enctype="multipart/form-data">
					...
					<div class="tab-pane" id="tab2">
						<p>
							<input type="file" name="files[0]"> <span
								class="help-block">Please upload only MP4 format videos
								only.</span>
						</p>
					</div>
					<div class="tab-pane" id="tab3">
						<p>
							<input type="file" name="files[1]"> <span
								class="help-block">Please upload only JPEG format images
								only.</span>
						</p>
					</div>
				</div>
	...
	<div id="uploadCover" class="modal hide fade" tabindex="-1"
		...
		<div class="modal-body">
			<p>
				<form:form method="post" action="uploadCtxCoverImage"
	...
	<div id="followCtx" class="modal hide fade" tabindex="-1" role="dialog"
		...
		<div class="modal-body">
			<p>
				<form:form method="post" action="followContext"
					...
					<fieldset>
						...
						<%-- <span class="help-block pull-right">  <a href="#">Know
								more about types.</a> 
						</span> --%>
						<span class="help-block pull-right">* Currently this and
	...
	<div id="inheritContextModal" class="modal hide fade" tabindex="-1"
		...
		<div class="modal-body">
			<form:form method="post" action="inheritContext"
				...
				<fieldset>
					<legend>Create a Funspot</legend>
	...
	<div class="container">
		...
		<c:choose>
					...
					<div class="row">
						<div class="card">
							<div class="card-heading image">
								<div class="card-heading-header">
									<span><b>Published by - <a
											href="${post.user.defaultContextId }">${currentContext.user.firstname },
												${currentContext.user.lastname }</a></b> at <fmt:formatDate
											value="${post.timestamp}" pattern="MM/dd/yyyy hh:mm a" /></span>
								</div>
							</div>
							<div class="card-body">
								<p>
									...
									<c:if test="${fn:length(post.iLocation) > 0}">
										<center>
											<a class="image-link"
												...
												data-lightbox="${ctxid }"> <img class="image"
												src="../uploads/images/${post.iLocation}.jpg" height="30%"
												width="70%" /></a>
										</center>
									</c:if>
								</p>
							...
							<div class="card-actions">
								<div class="btn-group">
									<a class="btn"
										href="${pageContext.servletContext.contextPath}/pst/Repost/${post.postId }/${ctxid}">Repost</a>
									<c:if test="${post.isOwner or currentContext.isOwner}">
										<button type="button"
										...
										<ul class="dropdown-menu pull-right" role="menu">
											<li><a
												href="${pageContext.servletContext.contextPath}/pst/deletePost/${post.postId }/${ctxid}">Delete</a></li>
										</ul>
									</c:if>
								</div>
							</div>
			...
			<c:otherwise>
				<div class="card">
					<div class="card-body">
						<p style="padding: 0px 0px 10px 0px">No posts found. Go to
							options and start posting.</p>
					</div>
				</div>
			</c:otherwise>
		</c:choose>
	...
	<div id="createContextModal" class="modal hide fade" tabindex="-1"
		...
		<div class="modal-body">
			<form:form method="post" action="createContext"
				commandName="contextBoard" name="createctx">
				<fieldset>
					<legend>Create a Funspot</legend>
					<label>Code:</label> <input type="text"
