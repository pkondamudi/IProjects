...
<head>
<meta charset="utf-8">
<title>${query }-{Marrker}Search</title>
...
<body>
	...
	<div class="container">
		...
		<c:choose>
			<c:when test="${fn:length(userSearchBoards) > 0}">
				<c:forEach var="userSearchBoard" items="${userSearchBoards}">
					<div class="row">
						<div class="card">
							<h3 class="card-heading simple">
								<c:choose>
									<c:when test="${userSearchBoard.isDefault eq 1 }">
										<i class="icon-user"></i>
									</c:when>
									<c:otherwise>
								...
								${userBoard.boardName }${userSearchBoard.boardName }
							</h3>
							<div class="card-body">
								<p>${userSearchBoard.boardDescription }</p>
								<p></p>
							</div>
							<div class="card-actions">
								<a class="btn" href="ctx/${userSearchBoard.boardId}">View</a>
							</div>
						</div>
					</div>
				</c:forEach>
				<ul class="pager">
					<li><a href="search?boardName=${query }&nav=pre">Previous</a></li>
					<li><a href="search?boardName=${query }&nav=nxt">Next</a></li>
				</ul>
			</c:when>
			<c:otherwise>
				<div class="card">
					<div class="card-body">
						<p style="padding: 0px 0px 10px 0px">
							No results found for search terms - <b>${query }</b>
						</p>
					</div>
				</div>
			</c:otherwise>
